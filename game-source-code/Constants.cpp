#include "Constants.h"

const float Constants::DISPLAY_WIDTH_= 800.0f;
const float Constants::DISPLAY_HEIGHT_ = 600.0f;
const float Constants::LASER_WIDTH_ = 1.0f;
const float Constants::LASER_HEIGHT_ = 7.0f;
const float Constants::PLAYER_WIDTH_ = 8;
const float Constants::PLAYER_HEIGHT_ = 8;
//const int Constants::PLAYER_SPEED_ = 5;
const float Constants::PLAYER_VERTICAL_LIMIT_ = 450.0f;
const float Constants::LASER_SPEED_ = 4.0f;
//const int Constants::SEGMENT_SPEED_ = 4;
//const int Constants::INITIAL_SIZE_OF_CENTIPEDE_ = 10;	//maximum possible is 26
//const int Constants::NUMBER_OF_MUSHROOMS = 10;  //prefereed value = 


